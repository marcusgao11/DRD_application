    ...
    do k=0,nz-1
       do i=0,nr
          ...
          fu(i,k)=fu(i,k) - (temp_p(i, k) - temp_p(i-1, k))/dr &
           &+ (rho(i,k)+rho(i-1,k))/2.0*( 2.0*u(i,k)*re/dt - u_pre(i,k)*re/2.0/dt + ggx )
       end do
    enddo
    ...
    do k=1,nz-1
       do i=0,nr-1
          fv(i,k)=fv(i,k) - (temp_p(i, k) - temp_p(i, k-1))/dz &
           & + (rho(i,k)+rho(i,k-1))/2.0*( 2.0*v(i,k)*re/dt - v_pre(i,k)*re/2.0/dt + ggz )
       end do
    enddo
...
!#ifdef FULL_IMPLICIT
    ...
    do k=0,nz-1
       do i=0,nr
          f11(i,k) = f1(i,k)*1.5*re*(rho(i,k) + rho(i-1,k))/2.0 &
          & + f1(i,k)* penalty_k* (bdr_ph(i,k) + bdr_ph(i-1,k))/2.0 *dt
          !& - dt*( (f1(i+1,k)+f1(i-1,k)-2*f1(i,k))/dr2 + (f1(i,k+1)+f1(i,k-1)-2*f1(i,k))/dz2 ) &
            !& *(eta(i,k) + eta(i-1,k))/2.0
        enddo
    enddo
    ...
    do k=1,nz-1
       do i=0,nr-1
          f22(i,k) = f2(i,k)*1.5*re* (rho(i,k) + rho(i,k-1))/2.0 &
          &  + f2(i,k)* penalty_k* (bdr_ph(i,k) + bdr_ph(i,k-1))/2.0 *dt
            !& - dt*( (f2(i+1,k)+f2(i-1,k)-2*f2(i,k))/dr2 + (f2(i,k+1)+f2(i,k-1)-2*f2(i,k))/dz2 ) &
            !& *(eta(i,k) + eta(i,k-1))/2.0
        enddo
    enddo
