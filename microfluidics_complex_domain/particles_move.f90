module particles_move
  ...
  subroutine particle_initialize(xc, yc, lx, ly, is_fluid) !N, l0)
    ...
    ! active force
    active_force   = 15 !5 !20.0 !1.0  !2.1250 !10 !0.
    if (.not. is_fluid) then
      active_force = 1.0
    endif

    active_force_ang   = 0.0 !PI/3
    ...
    !     write(*,*) "i", i, points(i, :)
    !     write(*,*) "Np-i", Np-i-1, points(Np-i-1, :)
    ...
    if (Np > 2) then
      i_num = floor(Np/2.0)
      points(i_num, 0) = xc  ! i_num = 7
      points(i_num, 1) = yc

      do i = 0, i_num-1
        points(i, 0) = xc
        points(i, 1) = yc + (i_num-i) * l0 !* 1.2

        points(Np-i-1, 0) = xc
        points(Np-i-1, 1) = yc - (i_num-i) * l0 !* 1.2
        write(*,*) "i", i, points(i, :)
        write(*,*) "Np-i", Np-i-1, points(Np-i-1, :)
...
!  points(1, 0) = 1.0 - 0.05
  ...
  do i = 0,Np_swm-1
      ...
      fpd_dist = min(fpd_dist, sqrt((x-pos_swm(i, 0))**2 + (y-pos_swm(i, 1))**2) - swm_radius)
  enddo
...
!  enddo
  ...
  subroutine force_caculation(dt,t, lx, ly, nr, nz, dr, dz, bdr_dist)
    use particles_params
    use particles
    implicit none
    real dt,t, lx,  ly
...
!    pause
    ...
    ! froce from bdr for fibers
    do i = 0, Np-1
        call interaction_force_bdr(nr, nz, dr, dz, bdr_dist, &
        & points(i,:), fpd_bdr_dist, k0_inter_bdr, f)
        tot_F(i,:) = tot_F(i,:) + f
    enddo
...
!    end if
  ...
  subroutine force_caculation_vel(nr, nz, dr, dz, r, z, lx, ly, &
      & I_thickness, bdr_dist, particle_force1, particle_force2,dt,t)
    use particles_params
    use particles
    implicit none
    ...
    call force_caculation(dt,t, lx, ly, nr, nz, dr, dz, bdr_dist)
...
!    points = points + dt * tot_F
  ...
  ! calculate interaction force on particle 1 from particle 2
  subroutine interaction_force_bdr(nr, nz, dr, dz, bdr_dist, &
    &pos_1, eq_dist, k_inter, force)
  !use particles
  implicit none
  integer nr,nz
  real:: dr, dz
  real bdr_dist(-1:nr,-1:nz)
  real pos_1(0:1), eq_dist, k_inter
  real force(0:1)

  integer i,j
  real d, di_x, di_y, di

    ! find nearest point and use the distance
    i = floor( (pos_1(0) - dr*0.5) / dr )
    j = floor( (pos_1(1) - dz*0.5) / dz )

    d = bdr_dist(i,j)
    di_x = (bdr_dist(i+1,j) - bdr_dist(i-1,j)) / 2.0 / dr
    di_y = (bdr_dist(i,j+1) -  bdr_dist(i,j-1)) / 2.0 /dz

    di = sqrt(di_x**2 + di_y**2)
    di_x = di_x / di; di_y = di_y / di;
      ...
      !write(*,*) d,eq_dist, force(0), force(1)
