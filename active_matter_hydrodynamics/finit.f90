...
!    call random_seed()
    do j=0,nz
      do i=0,nr
         ...
         z(i,j)=(j+0.5)*dz
         u(i,j)=0.0
         !u(i,j) = ( tanh(-(sqrt((r(i,j)-0.5*slen)**2 + (z(i,j)-shig*0.5)**2) - 0.05 )/(sqrt(2.0)*I_thickness) ) + 1.0 )/2.0 * 1.0
         v(i,j)=0.0
         p(i,j)=0.0
!         ph(i,j) = 1.0 
...
!do k=0,nz-1
!   do i=0,nr-1
!      rmu(i,k) = -I_thickness*( (ph(i+1,k)+ph(i-1,k)-2*ph(i,k))/dr2 &
!        + (ph(i,k+1)+ph(i,k-1)-2*ph(i,k))/dz2 )&
...
!do i_dt=0,50
!  call PhSolve(rmu_after,ph_after,0.0)
!  rmu = rmu_after
!  ph = ph_after
!enddo
!
...
!do k=0,nz-1
!   do i=0,nr-1
!      if (bdr_ph(i,k) >= 1.0) then
!        bdr_ph(i,k) = 1.0
!      else if (bdr_ph(i,k) <= 0.0) then
!        bdr_ph(i,k) = 0.0
!      endif
!   enddo
!enddo
