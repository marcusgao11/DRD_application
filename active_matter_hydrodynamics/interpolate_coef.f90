...
!    enddo
    ...
    do k=0,nz-1
      do i=0,nr-1
        ph_swm(i,k) = tanh( -1.0* fpd_dist_swm(r(i,k), z(i,k), slen, shig) /sqrt(2.0)/I_thickness )
        !dist(r(i,k), z(i,k)) !tanh( -1.0*dist(r(i,k), z(i,k)) /sqrt(2.0)/I_thickness )
      enddo
    enddo
    ph_swm = (ph_swm + 1.0)/2.0
    do k=0,nz-1
      ph_swm(-1,k) = ph_swm(0,k)
      ph_swm(nr,k) = ph_swm(nr-1,k)
     enddo
 
     do i=-1,nr
      ph_swm(i,-1) = ph_swm(i,0)
      ph_swm(i,nz) = ph_swm(i,nz-1)
     end do
    ...
    do k=0,nz-1
      do i=0,nr-1
         ph(i,k) = tanh( -1.0*dist(r(i,k), z(i,k)) /sqrt(2.0)/I_thickness )
         !dist(r(i,k), z(i,k)) !tanh( -1.0*dist(r(i,k), z(i,k)) /sqrt(2.0)/I_thickness )
      enddo
    enddo
  ...
  !           eta(i,k) = 1.0 + (lambda_eta-1.0)*ph(i,k)
  !       enddo
  ! enddo
