...
!#define SEMI_IMPLICIT
  ...
  real u_after(-1:nr+1,-1:nz)
  real v_after(-1:nr,0:nz)
...
!     ! TODO: force with density? and coefficient for scaling
!    do k=0,nz-1
!       do i=0,nr
...
!    enddo
  ...
  real cl, cr ! temporay coefficients, cl: for coef left, cr: coef right
    ...
    do i=-1,nr
     f2(i,0) = (f2(i,nz-1) + f2(i,1))/2.0
     f2(i,nz) = (f2(i,nz-1) + f2(i,1))/2.0
    enddo
    ...
    do k=0,nz-1
       do i=0,nr
          f11(i,k) = f1(i,k)*re*(rho(i,k) + rho(i-1,k))/2.0 &
            & + f1(i,k)* penalty_k* (1.0 + (ph(i,k) + ph(i-1,k))/2.0) /2.0 *dt
            !& - dt*( (f1(i+1,k)+f1(i-1,k)-2*f1(i,k))/dr2 + (f1(i,k+1)+f1(i,k-1)-2*f1(i,k))/dz2 ) &
            !& *(eta(i,k) + eta(i-1,k))/2.0
        enddo
    enddo
    ...
    do k=1,nz-1
       do i=0,nr-1
          f22(i,k) = f2(i,k)*re* (rho(i,k) + rho(i,k-1))/2.0 &
             &  + f2(i,k)* penalty_k* (1.0 + (ph(i,k) + ph(i,k-1))/2.0) / 2.0 *dt
            !& - dt*( (f2(i+1,k)+f2(i-1,k)-2*f2(i,k))/dr2 + (f2(i,k+1)+f2(i,k-1)-2*f2(i,k))/dz2 ) &
            !& *(eta(i,k) + eta(i,k-1))/2.0
        enddo
    enddo
...
!    do j=0,nz-1
!       do i=0,nr
!         tmp = re*(rho(i,j) + rho(i-1,j))/2.0 + penalty_k* (1.0 + (ph(i,j) + ph(i-1,j))/2.0) /2.0 *dt
!
!         cr = eta(i,j)
!         cl = eta(i-1,j)
...
!    do j=1,nz-1
!       do i=0,nr-1
!        tmp = re*(rho(i,j) + rho(i-1,j))/2.0 + penalty_k* (1.0 + (ph(i,j) + ph(i,j-1))/2.0) / 2.0 *dt
!
!         cr = eta(i,j)
!         cl = eta(i,j-1)
